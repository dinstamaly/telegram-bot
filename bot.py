...
from key_buttons import tele_button, nav_button, credit1_menu, deposit_menu, credit2_menu, credit3_menu, credit4_menu, \
    currency
from button_dict import dep_menu, cred_menu, info_button

from menu_keyboards import main_menu_keyboard, atm_menu_keyboard, depo_menu_keyboard, deposits_menu_keyboard, \
    credits_menu_keyboard, potreb_menu_keyboard, ipoteka_menu_keyboard, auto_menu_keyboard, currency_menu_keyboard
from messages import m_atm, m_depo, m_auto, m_main, m_credit, m_deposit, m_ipoteka, m_potreb, m_currency
    ...
    if list(context.user_data.values())[-1] == ('Банкоматы',):
        ...
        r = requests.post(atm_url,
                          json={
                              ...
                              "type_code": "BANKOMAT"
                          }, headers={"Authorization": "Basic MTE6MQ==", "Content-Type": "application/json"},
                          verify=False)
        # получаем данные
        json_data = r.json()

        for atm in json_data.get('atm_list', []):
            msg = context.bot.send_message(chat_id=update.effective_chat.id, text=atm["name"] + "\n" + atm["address"])
            context.bot.sendLocation(chat_id=update.effective_chat.id, longitude=atm["long"], latitude=atm["lat"],
...
INFO_REGEX = r"(?=(" + (tele_button[2]) + '|' + (tele_button[6]) + '|' + (tele_button[7]) + r"))"
...
def code_info(update: Update, context: CallbackContext):
    ...
    for currency in json_data.get('currencies', []):
        if info[0] == currency["code"]:
            update.message.reply_text(f"{currency['code']}: {currency['buy']} - {currency['sell']}\n"
...
updater.dispatcher.add_handler(MessageHandler(Filters.regex(ATM_REGEX), receive_atm_location))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(DEPO_REGEX), receive_depo_location))

updater.dispatcher.add_handler(MessageHandler(Filters.regex(INFO_REGEX), receive_info))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(DEPOSITS_REGEX), receive_deposit_option))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(D_INFO_REGEX), receive_deposit_info))

updater.dispatcher.add_handler(MessageHandler(Filters.regex(CREDITS_REGEX), receive_credit_option))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(CREDITS_POTREB_REGEX), receive_potreb_option))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(CREDITS_IPOTEKA_REGEX), receive_ipoteka_option))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(CREDITS_AUTO_REGEX), receive_auto_option))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(C_1_INFO_REGEX), receive_potreb_info))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(C_2_INFO_REGEX), receive_ipoteka_info))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(C_3_INFO_REGEX), receive_auto_info))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(CURRENCY_REGEX), receive_currency_option))
updater.dispatcher.add_handler(MessageHandler(Filters.regex(CURRENCY_CODE_REGEX), code_info))

updater.dispatcher.add_handler(MessageHandler(Filters.regex(HOME_REGEX), back_main_menu))
